    ...
    with torch.no_grad():
        while 1:
            ...
            while 1:
                ...
                print('mask shape = ', len(input_ids[0]))
                print('mask shape =', input_ids.shape)
                attention_a = torch.ones(1, len(input_ids[0]))
                # attention_b = torch.zeros(1, 40 - len(input_ids[0]))
                
                
                print('attention_a shape =', attention_a)
                # print('attention_b shape =', attention_b)
                # attention_mask = torch.cat((attention_a,attention_b), dim=1)
                
                pred = model(input_ids.to(device), attention_mask = attention_a.to(device))
                pred = pred.logits
